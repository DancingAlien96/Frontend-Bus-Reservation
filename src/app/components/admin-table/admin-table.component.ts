...
import { SolicitudesInterfaces } from '../../shared/interfaces/solicitudes-interfaces';
...
export class AdminTableComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['ID_SOLICITUD', 'FECHA_CREACION', 'FECHA_HORA_ENTREGA', 'FECHA_HORA_DEVOLUCION', 'USUARIO.NOMBRE_COMPLETO', 'ESTADO'];
...
this.solicitudesService.getSolicitudes().subscribe(data=>{
  ...
  this.dataSource.filterPredicate = (data: SolicitudesInterfaces, filter: string) => {
    ...
    const dataStr = `${data.ID_SOLICITUD} ${data.FECHA_CREACION} ${data.FECHA_HORA_ENTREGA} ${data.FECHA_HORA_DEVOLUCION} ${data.USUARIO?.NOMBRE_COMPLETO} ${estadoLabel}`.toLowerCase();
  ...
  openDialog(row: SolicitudesInterfaces) {
    ...
    this.dialog.open(PopupComponent, {
      ...
      data: { id: row.ID_USUARIO ,
              usuario: row.USUARIO.NOMBRE_COMPLETO
       },
    });
  }
}
